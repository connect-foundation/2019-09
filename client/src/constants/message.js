const MAIN_HOW_TO_PLAY_TITLE = 'How to Play';
const MAIN_HOW_TO_PLAY_DESCRIPTION = `자신의 차례가 되면, 세 가지 단어 중에서 한 가지를 선택하고 80 초 안에 그 단어를 몸으로 표현해야 합니다. 또는 다른 누군가가 몸으로 표현할 때 포인트를 얻으려면 채팅으로 정답을 추측해서 입력해야 합니다. 랭킹에 오르기 위해서는 최대한 많은 정답을 맞히세요!`;
const MAIN_SLOGAN = `"Talk is cheap. Show me the move."`;

const DEFAULT_SCOREBOARD_TITLE = 'Final Score';
const WAITING_FOR_STREAMER = 'Waiting For Streamer...';
const GAME_END_SCOREBOARD_TITLE = '게임 종료';
const ALLOW_CAMERA_MESSAGE = '카메라를 허용해주세요';

const COPY_TO_CLIPBOARD_MESSAGE = 'is copied to your clipboard';

const ROOM_UNAVAILABLE_MESSAGE = '방에 연결이 불가능 합니다.';

export {
  MAIN_HOW_TO_PLAY_TITLE,
  MAIN_HOW_TO_PLAY_DESCRIPTION,
  MAIN_SLOGAN,
  DEFAULT_SCOREBOARD_TITLE,
  WAITING_FOR_STREAMER,
  GAME_END_SCOREBOARD_TITLE,
  ALLOW_CAMERA_MESSAGE,
  COPY_TO_CLIPBOARD_MESSAGE,
  ROOM_UNAVAILABLE_MESSAGE,
};
